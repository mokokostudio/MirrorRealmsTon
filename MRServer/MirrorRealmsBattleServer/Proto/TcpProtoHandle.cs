...
namespace MR.Net.Proto {
    ...
    public class TcpProtoHandle {
        ...
        public void Regist<T>(Action<TcpProtoHandle, T> callback) {
            if (!CheckType<T>())
                return;
        ...
        public void RegistOnce<T>(Action<TcpProtoHandle, T> callback) {
            if (!CheckType<T>())
                return;
            var msg = s_Type2MsgDic[typeof(T)];
        ...
        public void UnRegist<T>(Action<TcpProtoHandle, T> callback) {
            if (!CheckType<T>())
                return;
            var msg = s_Type2MsgDic[typeof(T)];
            if (m_CallBacksP.TryGetValue(msg, out var d))
        ...
        public void Send<T, U>(T obj, Action<TcpProtoHandle, U> callback) {
            Send(obj);
            RegistOnce(callback);
        }
        ...
        private void AddCall(Delegate callback, object obj) {
            try { callback.DynamicInvoke(this, obj); } catch { }
        }
