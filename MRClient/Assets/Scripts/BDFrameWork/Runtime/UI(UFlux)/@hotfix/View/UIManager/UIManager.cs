...
namespace BDFramework.UFlux
{
    ...
    public partial class UIManager : ManagerBase<UIManager, UIAttribute>
    {
        ...
        public void LoadWindow(Enum uiIndex, Action callBack = null)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    window.AsyncLoad(() =>
                    {
                        window.Transform.gameObject.SetActive(index == 0);

                        Debug.Log($"加载界面{index}");
        ...
        public void AsyncLoadWindow(Enum uiIndex, Action callback)
        {
            ...
            if (windowMap.ContainsKey(index))
            {
                ...
                if (uvalue.IsLoad)
                {
                    UnityEngine.Debug.Log("已经加载过并未卸载");
                }
            }
            else
            {
                ...
                if (window == null)
                {
                    UnityEngine.Debug.Log("不存在UI:" + index);
                }
                else
                {
                    windowMap[index] = window as IWindow;
        ...
        public void ScalMinWindow(Enum uiEnumIdx)
        {
            int uiIdx = uiEnumIdx.GetHashCode();
            if (windowMap.ContainsKey(uiIdx))
            {
                ...
                winCom.Transform.GetComponent<RectTransform>().anchorMin = new Vector2(2, 2);
            }
        }

        public void ScalMaxWindow(Enum uiEnumIdx)
        {
            int uiIdx = uiEnumIdx.GetHashCode();
            if (windowMap.ContainsKey(uiIdx))
            {
                var win = windowMap[uiIdx];
                var winCom = win as IComponent;
                winCom.Transform.GetComponent<RectTransform>().anchorMin = new Vector2(0, 0);
            }
        }
        ...
        public void Forward()
        {
            ...
            {
                UnityEngine.Debug.LogError("已经是顶部");
            }
        }
        ...
        public void Back()
        {
            ...
            {
                UnityEngine.Debug.LogError("已经是底部");
            }
        }
        ...
        private void PushCaheData(int uiIdx)
        {
            ...
            if (cacheList != null)
            {
                for (int i = 0; i < cacheList.Count; i++)
                {
                    ...
                    UnityEngine.Debug.Log("push cache data " + uiIdx);
                }

                cacheList.Clear();
                UnityEngine.Debug.LogFormat("推送数据：{0} ,{1}条", uiIdx, cacheList.Count);
            }
        }
